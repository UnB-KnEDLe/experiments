...
# from drive.MyDrive.partial_DAL.crf import CRF
from crf import CRF
# from crf import CRF
...
class bilstm_crf(nn.Module):
    ...
    def forward(self, x, y, mask, soft_label=False):
        x = self.dropout(x)
        x, (_, _) = self.bilstm(x)
        x = self.dropout(x)
        x = self.linear(x)
        x = self.crf(x, y, mask=mask, soft_label=soft_label)
        return x
...
class cnn_bilstm_crf(nn.Module):
    def __init__(self, char_vocab_size, char_embedding_dim, char_out_channels, pretrained_word_emb, num_classes, device, lstm_hidden_size):
        ...
        self.word_embedder= nn.Embedding.from_pretrained(torch.FloatTensor(pretrained_word_emb.vectors), freeze=False)
