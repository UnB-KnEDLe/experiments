...
def valid_text(text, min_len=4, max_spacing=3):
    """Returns whether or not `text` is upper and other things.

    Evaluates to true if d['text'] matches the following conditions:
        - all letters are uppercase
        - does not contain 4 or more consecutive spaces
        - has a lenght greater than `min_len`
    Args:
        text: string to be matched against conditions
        min_len: `int`. Specify minimum length `text ` should be
        max_spacing: `int`. Max contiguous spacing allowed
    ."""
    t = text.strip(' .')
    cond1 = (max_spacing + 1) * ' ' not in t
    cond2 = len(t) >= min_len
...
def is_title_subtitle(span):
    ...
    return ( valid_text(text)
            ...
            and not re.search(_TRASH_COMPILED, text)
...
def hierarchy_text(hierarchy):
    texts = {}
    for k, v in hierarchy.items():
        texts[k] = []
        for title, rest in v:
            for _, blocks in rest:
                texts[k].extend(blocks)
    return texts
