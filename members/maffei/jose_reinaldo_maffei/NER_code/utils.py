...
def load_embedding(parser_opt, base_path: Path, df: pd.DataFrame = None, tokens_col='ato'):
    if parser_opt.dataset == 'conll':
        ...
        train_path = base_path / 'ontonotes/good_splits/onto.train.ner'
        if parser_opt.use_dev_set:
            test_path = base_path / 'ontonotes/good_splits/onto.development.ner'
        else:
            test_path  = base_path / 'ontonotes/good_splits/onto.test.ner'
        data_format = 'iob2'
        embedding_path = ''
        emb = KeyedVectors.load(embedding_path.as_posix())
        ...
        if parser_opt.use_dev_set:
            ...
            test_path  = base_path / 'aposentadoria/aposentadoria_testb.txt'
        data_format = 'iob2'
