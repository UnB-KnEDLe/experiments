...
class CNN_CNN_CRF(nn.Module):
    """
    CNN_CNN_CRF model with greedy decoding (as proposed in Maffei's Head)
    """
    def __init__(self, char_vocab_size, char_embedding_dim, char_out_channels, word2idx, 
        ...
        super(CNN_CNN_CRF, self).__init__()
        self.num_classes = num_classes
        ...
        self.dropout = nn.Dropout(p=0.5)
        self.linear = torch.nn.Linear(
            in_features=char_out_channels+word_out_channels, 
            out_features=num_classes)
    ...
    def forward(self, sentence, word, tag, mask):
        ...
        word_emb = self.word_encoder(sentence, char_emb)
