...
from CNN_CNN_LSTM_process_data import CustomDropout
    ...
    def fit(self, epoch, train_loader, test_loader, tag2idx, lr, momentum, clipping_value):
        ...
        for it in range(epoch):
            ...
            for sent, tag, word, mask in train_loader:
                sent = sent.to(self.decoder.device)
                tag = tag.to(self.decoder.device)
                word = word.to(self.decoder.device)
                mask = mask.to(self.decoder.device)
                optim.zero_grad()
            ...
            self.eval()
            with torch.no_grad():
                predictions, targets = preprocess_pred_targ(self, test_loader, self.decoder.device)
